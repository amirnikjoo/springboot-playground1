package com.kian.branch.web.rest;
...
import com.kian.branch.config.KafkaProperties;
...
class BranchKafkaResourceIT {
    ...
    void setup() {
        ...
        BranchKafkaResource kafkaResource = new BranchKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/branch-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/branch-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
