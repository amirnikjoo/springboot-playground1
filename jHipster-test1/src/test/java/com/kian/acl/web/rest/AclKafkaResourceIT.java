...
import com.kian.acl.config.KafkaProperties;
...
class AclKafkaResourceIT {
    ...
    void setup() {
        ...
        AclKafkaResource kafkaResource = new AclKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/acl-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/acl-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
