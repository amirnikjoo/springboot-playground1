...
public class FetchRemoveAccountRequests extends IHandler {
    @Setter
    ...
    @Setter
    private IHandler offlineRemoveAccountFlow;
    ...
    public void process(Map map) throws Exception {
        ...
        List<WalletRequest> requests = walletRequestRepository.getWalletRequestByHandledAndServiceId(
                WalletConstants.HANDLE, WalletConstants.SERVICE_ID_REMOVE_ACCOUNT);
        if (requests.size() > 0)
    ...
    private class MySession implements Runnable {
        ...
        public void run() {
            ...
            try {
                offlineRemoveAccountFlow.process(map);
