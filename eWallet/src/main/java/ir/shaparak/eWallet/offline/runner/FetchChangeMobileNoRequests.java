...
public class FetchChangeMobileNoRequests extends IHandler {
    @Setter
    ...
    @Setter
    private IHandler offlineChangeMobileNoFlow;
    ...
    public void process(Map map) throws Exception {
        ...
        List<WalletRequest> requests = walletRequestRepository.getWalletRequestByHandledAndServiceId(
                WalletConstants.HANDLE, WalletConstants.SERVICE_ID_CHANGE_MOBILE_NO);
        if (requests.size() > 0)
    ...
    private class MySession implements Runnable {
        ...
        public void run() {
            ...
            try {
                offlineChangeMobileNoFlow.process(map);
