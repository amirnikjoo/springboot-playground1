...
public class FetchChangeRiskLevelRequests extends IHandler {
    @Setter
    ...
    @Setter
    private IHandler offlineChangeLevelFlow;
    ...
    public void process(Map map) throws Exception {
        ...
        List<WalletRequest> requests = walletRequestRepository.getWalletRequestByHandledAndServiceId(
                WalletConstants.HANDLE, WalletConstants.SERVICE_ID_CHANGE_RISK_LEVEL);
        if (requests.size() > 0)
    ...
    private class MySession implements Runnable {
        ...
        public void run() {
            ...
            try {
                offlineChangeLevelFlow.process(map);
