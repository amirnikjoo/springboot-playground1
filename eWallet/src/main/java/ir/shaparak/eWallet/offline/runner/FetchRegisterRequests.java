...
public class FetchRegisterRequests extends IHandler {
    @Setter
    ...
    @Setter
    private IHandler offlineRegisterFlow;
    ...
    public void process(Map map) throws Exception {
        ...
        List<WalletRequest> requests = walletRequestRepository.getWalletRequestByHandledAndServiceId(
                WalletConstants.HANDLE, WalletConstants.SERVICE_ID_REGISTER);
        if (requests.size() > 0)
    ...
    private class MySession implements Runnable {
        ...
        public void run() {
            ...
            try {
                offlineRegisterFlow.process(map);
